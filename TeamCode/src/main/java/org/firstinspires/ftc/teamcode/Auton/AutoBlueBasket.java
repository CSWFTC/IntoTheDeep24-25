...
public class AutoBlueBasket extends LinearOpMode {
    ...
    private void toPosThree(){
        ...
        Action moveThree = drive.actionBuilder(drive.pose)
                .setReversed(false)
                .splineTo(new Vector2d(-21.2, -53.0), Math.toRadians(-55))
                .build();
    ...
    private void toBasket(){
        ...
        Action moveBasket= drive.actionBuilder(drive.pose)
                ...
                .build();
        Actions.runBlocking(new SequentialAction(moveBasket, vip.dumpSampleHighBasket(), bucket.autonPrepForCatch(), vip.autonReset()) );
    }
